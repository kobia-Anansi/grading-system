...
int main(){
    ...
    for (int i = 0; i < 6; i++){ //each scope within the for loop returns 0 so that when the grade for the value inputed
        if (x >= min and x <= max){ // is found, the for loop discontinues to reduce program execution time.
            cout << "You had a " << grade[i] << endl;
            return 0;
        }else if (x < 50 or x == 0){
